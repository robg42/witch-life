...
function Section({
  ...
  return (
    <section className="mt-10 first:mt-0">
      <h2 className="font-sans text-xs uppercase tracking-[0.25em] text-ash mb-3">
        {label}
      </h2>
      <div className="oracle-body whitespace-pre-wrap text-parchment/95">
        {children}
      </div>
    </section>
  );
...
function NatalReport({ data }: { data: NatalReportJson }) {
  return (
    <article>
      <p className="oracle-body text-parchment/95">{data.summary}</p>
...
function YearAheadReport({ data }: { data: YearAheadReportJson }) {
  return (
    <article>
      <p className="oracle-body text-parchment/95">{data.opening}</p>
      {data.months?.map((m) => (
        <section key={m.month} className="mt-10">
          <h2 className="accent text-2xl text-ochre">{m.month}</h2>
          <p className="oracle-body mt-3 whitespace-pre-wrap text-parchment/95">
            {m.narrative}
          </p>
          {m.watchFor && (
            <p className="mt-3 font-sans text-xs uppercase tracking-[0.25em] text-sage">
        ...
        <div className="space-y-6">
          {data.eclipses?.map((e, i) => (
            <div key={i}>
              ...
              <p className="oracle-body mt-1 text-parchment/95">
                {e.whatItDisturbs}
              </p>
            </div>
          ))}
