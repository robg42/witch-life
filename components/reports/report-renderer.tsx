...
function Section({
  ...
  return (
    <section className="mt-10 first:mt-0">
      ...
      <div className="oracle-body whitespace-pre-wrap text-wax/95">
        {children}
      </div>
    </section>
  );
...
function NatalReport({ data }: { data: NatalReportJson }) {
  return (
    <article>
      <p className="oracle-body text-wax/95">{data.summary}</p>
...
function YearAheadReport({ data }: { data: YearAheadReportJson }) {
  return (
    <article>
      <p className="oracle-body text-wax/95">{data.opening}</p>
      {data.months?.map((m) => (
        <section key={m.month} className="mt-10">
          <h2 className="accent text-2xl text-clay">{m.month}</h2>
          <p className="oracle-body mt-3 whitespace-pre-wrap text-wax/95">
            {m.narrative}
          </p>
          {m.watchFor && (
        ...
        <div className="space-y-6">
          {data.eclipses?.map((e, i) => (
            <div key={i}>
              ...
              <p className="oracle-body mt-1 text-wax/95">
                {e.whatItDisturbs}
              </p>
            </div>
          ))}
